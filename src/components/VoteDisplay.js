...
function VoteDisplay(props) {
    ...
    if (turn !== myHouse && !votes[players.house]) {
        ...
        const showAddPower = (votes[myHouse] && (votes[myHouse].type === "aye" || votes[myHouse].type === "nay")) ||
            (intermediateVote.vote === "aye" || intermediateVote.vote === "nay") ? true : false;
        ...
        content = <div className={`add-power-container ${intermediateVote["color"]}`}>
            ...
            {
                (showAddPower)
                && <> <h2 className="add-power add-power-header">Current Power Comitted: {currentPowerComitted}</h2>
